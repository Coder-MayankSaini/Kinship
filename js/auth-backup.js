...
class AuthManager {
    ...
    loadCurrentUser() {
        try {
            const userId = window.KinshipStorage.getData('kinship_current_user');
            console.log('AuthManager: Loading current user with ID:', userId);

            if (userId) {
                this.currentUser = window.KinshipStorage.getUser(userId);
